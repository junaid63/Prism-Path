...
class ReportCommand extends Command
{
    ...
    protected $description = 'Generate scheduled UltraClarity analytics reports and optionally email them.';
    ...
    public function handle(AnalyticsRepository $analytics): int
    {
        ...
        if ($this->option('email')) {
            foreach (config('ultraclarity.reports.recipients', []) as $recipient) {
                Mail::raw("Your UltraClarity {$frequency} report is ready: {$path}", function ($message) use ($recipient, $frequency): void {
                    $message->to($recipient)->subject('UltraClarity ' . ucfirst($frequency) . ' Report');
                });
            }
        }
