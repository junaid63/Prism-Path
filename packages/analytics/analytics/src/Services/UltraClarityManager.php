...
class UltraClarityManager
{
    ...
    public function script(array $options = []): string
    {
        ...
        return '<script>window.UltraClarityConfig = ' . $payload . ';</script><script src="' . e(route('ultraclarity.snippet')) . '" ' . ($config['async'] ? 'async ' : '') . ($config['defer'] ? 'defer' : '') . '></script>';
    }
}
